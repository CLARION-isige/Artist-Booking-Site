...
# TODO: connect to a local postgresql databas
app.config['SQLALCHEMY_DATABASE_URI']= 'postgresql://postgres:39095780@localhost:5432/testdb'
...
class Venue(db.Model):
    ...
    count_upcoming_shows = db.Column(db.Integer, default=0)
    count_past_shows = db.Column(db.Integer, default=0)
...
class Show(db.Model):
  ...
  show_start_time = db.Column(String(), nullable=False)
...
@app.route('/')
def index():
  recent_artist = Artist.query.order_by(db.desc(Artist.id)).limit(10).all()
...
def venues():
  ...
  for area in venue_areas:
    venues = db.session.query(Venue.id, Venue.name,
              Venue.count_upcoming_shows).filter(Venue.city==area[0], Venue.state==area[1]).all()
    data.append({
      "city": area[0],
      "state": area[1],
      "venues": []
...
def search_venues():
  ...
  for venue in venue_results:
    response["data"].append({
      "id": venue.id,
      "name": venue.name,
      "num_upcoming_shows": venue.count_upcoming_shows
    })
...
def delete_venue(venue_id):
  ...
  try:
    ...
    db.session.rollback()
    flash('please try the operation again. Venue '+ venue_name + ' could not be deleted.')
  finally:
    db.session.close()
...
def search_artists():
  ...
  for artist in query_artist:
    response['data'].append({
      "id": artist.id,
      "name": artist.name,
      "num_upcoming_shows": artist.count_upcoming_shows
    })
...
def show_artist(artist_id):
  ...
  for show in shows:
    show_info = {
      ...
      "start_time": str(show.show_start_time)
    }
    if(show.upcoming):
...
def create_show_submission():
  ...
  try:
    ...
    show.show_start_time = request.form['show_start_time']
    db.session.add(show)
    db.session.commit()
  except:
    error= True
